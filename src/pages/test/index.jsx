...
export function IdCardScanner() {
      ...
      const worker = await createWorker({
        logger: (progress) => {
          if (progress.status === "recognizing text") {
            setProgress(Math.min(100, Math.max(0, progress.progress * 100)));
          }
        },
      });
  ...
  const parseIdCardText = (text) => {
    ...
    const dobPattern =
      /\b(?:DOB|Date of Birth|Birth Date)[.:# ]?[ ]?([0-9]{1,2}[/.-][0-9]{1,2}[/.-][0-9]{2,4})\b/i;
    const expiryPattern =
      /\b(?:Expiry|Expiration|Exp)[.:# ]?[ ]?([0-9]{1,2}[/.-][0-9]{1,2}[/.-][0-9]{2,4})\b/i;
    ...
    for (let i = 0; i < lines.length; i++) {
      if (
        lines[i].match(/\b(?:Address|Addr)[.:# ]?\b/i) &&
        i + 1 < lines.length
      ) {
  ...
  return (
    <Card className='w-full'>
      <CardHeader>
        ...
        <CardDescription>
          Upload an ID card image to extract information using OCR
        </CardDescription>
      </CardHeader>
                  ...
                  {Object.keys(extractedData).length > 0 ? (
                    <div className='grid gap-2'>
                      {Object.entries(extractedData).map(([key, value]) => (
                        <div
                          key={key}
                          className='grid grid-cols-3 gap-2 py-1 border-b last:border-0'>
                    ...
                    <p className='text-sm text-muted-foreground'>
                      No structured information could be extracted. Try
                      adjusting the image or using a clearer photo.
                    </p>
                  )}
                </div>
      ...
      <CardFooter className='flex justify-between'>
        ...
        {image && !isProcessing && !extractedText && (
          <Button onClick={processImage}>Extract Information</Button>
        )}
      </CardFooter>
    </Card>
  );
